...
test.describe('Calendario E2E Tests', () => {
  ...
  test('navega entre meses correctamente', async ({ page }) => {
    ...
    const currentMonth = await page.getByTestId('calendar-month-name').textContent();

    // Navegar al mes siguiente
    await page.getByTestId('calendar-next-btn').click();

    // Verificar que cambió el mes
    const nextMonth = await page.getByTestId('calendar-month-name').textContent();
    ...
    const backToOriginal = await page.getByTestId('calendar-month-name').textContent();
  ...
  test('carga y muestra eventos de citas', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      return document.querySelectorAll('[data-testid*="calendar-event"]').length > 0;
    }, { timeout: 10000 });
  ...
  test('cierra modal de edición correctamente', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      return document.querySelectorAll('[data-testid*="calendar-event-cita"]').length > 0;
    }, { timeout: 10000 });
...
test('verificación básica del calendario - FALLBACK', async ({ page }) => {
  ...
  for (const indicator of calendarIndicators) {
    if (await indicator.count() > 0) {
      ...
      foundIndicator = true;
      console.log(`✅ Encontrado indicador de calendario: ${await indicator.first().textContent()}`);
      break;
    }
  }
  ...
  if (!foundIndicator) {
    ...
    console.log('⚠️  No se encontraron indicadores específicos del calendario, pero la página cargó');

    // Tomar screenshot para debug
    await page.screenshot({ path: 'calendar-fallback-debug.png', fullPage: true });
