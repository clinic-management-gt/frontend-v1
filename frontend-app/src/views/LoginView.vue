<template>
  <div class="flex min-h-screen flex-1 flex-col justify-center items-center px-6 py-12 lg:px-8">
    ...
    <form @submit.prevent="onSubmitLogin" class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm space-y-4">
      <text-input class="mt-2" name="email" type="email" title="login.mail"
        inputPlaceholder="login.placeholder-mail" inputColor="primary-color" />

      <text-input class="mt-2" name="password" type="password" title="login.password"
        inputPlaceholder="login.placeholder-password" inputColor="primary-color" />

      <div v-if="error !== null" class="text-red-600 text-sm">{{ error }}</div>

      <primary-button @click="onSubmitLogin"  type="submit" :disabled="isLoadingAuthStore || !isFormValid" class="w-full flex justify-center">
