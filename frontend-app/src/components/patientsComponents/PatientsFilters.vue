      ...
      <div class="mx-auto flex max-w-7xl divide-x divide-gray-200 px-4 text-sm sm:px-6 lg:px-8">
        <div class="pr-6">
          <DisclosureButton class="group flex items-center font-medium text-gray-700">
            <funnel-icon
              class="mr-2 size-5 flex-none text-emerald-600 group-hover:text-gray-500"
              aria-hidden="true"
            />
        ...
        >
          <div class="flex flex-col">
            <p class="mb-2 text-gray-600 block text-sm font-medium leading-6 truncate">
              {{
                $t('protocol-instruments.writing-number') }}
            </p>
            <search-option
              :cleanText="clearText"
              icon="MagnifyingGlassIcon"
              textPlaceholder="protocol-instruments.writing-number"
              @search="updateSearchCriteria('writingNumber', $event)"
            />
          </div>
          <div class="flex flex-col">
            <p class="mb-2 text-gray-600 block text-sm font-medium leading-6 truncate">
              {{
                $t('protocol-instruments.customer') }}
            </p>
            <search-option
              :cleanText="clearText"
              icon="MagnifyingGlassIcon"
              textPlaceholder="protocol-instruments.customer"
              @search="updateSearchCriteria('customer', $event)"
            />
          </div>
          <div class="flex flex-col">
            <p class="mb-2 text-gray-600 block text-sm font-medium leading-6 truncate">
              {{
                $t('protocol-instruments.type-of-instrument') }}
            </p>
            <search-option
              :clearText="clearText"
              icon="MagnifyingGlassIcon"
              textPlaceholder="protocol-instruments.type-of-instrument"
              @search="updateSearchCriteria('instrumentType', $event)"
            />
          </div>
          <div class="flex flex-col">
            <p class="mb-2 text-gray-600 block text-sm font-medium leading-6 truncate">
            ...
            <search-option
              :cleanText="clearText"
              icon="MagnifyingGlassIcon"
              textPlaceholder="protocol-instruments.state"
              @search="updateSearchCriteria('state', $event)"
            />
          </div>
          <div class="flex flex-col ">
