     ...
     <ul role="list" class="flex flex-1 flex-col gap-y-7">
               ...
               <ul role="list" class="-mx-2 space-y-1">
                    <li v-for="(item, index) in navigationObject" :key="item">
                         <a v-if="!item.children" :href="item.href" @click="placeCurrent(item)" :class="[
                              item.current ? 'bg-[#]' : 'hover:bg-[#82C0CC]',
                         ...
                         <div v-else>
                              <button
                                   :class="[openIndex === index ? 'bg-emerald-600' : 'hover:bg-emerald-900', 'flex items-center w-full text-left rounded-md p-2 gap-x-1 text-sm leading-6 font-semibold text-white']"
          ...
          <div class="relative">
               <div class="absolute inset-0 flex items-center" aria-hidden="true">
                    <div class="w-full border-t border-[#6FBDCF]" />
               </div>
          </div>
