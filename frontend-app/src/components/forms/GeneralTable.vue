        ...
        <div class="inline-block min-w-full align-middle sm:px-4">
          <table class="min-w-full divide-y divide-gray-300">
                ...
                >
                  ...
                  <button v-else-if="header.hasAction" class="flex justify-start py-2 px-2">
                    <action-button-solid-icon
                      v-if="header.buttonType === 'solidIcon'"
                      :icon="header.iconType"
                      size="h-8 w-8"
                      @click="header.action"
                    />
                    <action-button-outlined-icon
                      v-if="header.buttonType === 'outlineIcon'"
                      :icon="header.iconType"
                      class="h-6 w-6 mr-1"
                      @click="header.action"
                    />
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              ...
              <tr v-for="(item, index) in displayedItems" v-else :key="index">
                <td
                  v-for="header in props.data.headers.filter(h => h.visible)"
                  :key="header.key"
