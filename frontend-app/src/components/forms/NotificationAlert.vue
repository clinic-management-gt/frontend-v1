  ...
  <Teleport to="body">
    ...
    class="fixed inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6 z-[9999] pointer-events-none">
      <div class="flex w-full flex-col items-center space-y-4 sm:items-end">
        <transition v-for="(n, index) in notifications" :key="index"
        ...
        leave-to-class="opacity-0">
          <div v-if="n.visible"
          :class="['pointer-events-auto cursor-default w-full max-w-sm overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5', styleFor(n.type).card]"
          @mouseenter="notificationStore.startHovering(n.id)"
          @mouseleave="notificationStore.stopHovering(n.id)">
            <div class="p-4">
              <div class="flex items-center">
                <component :is="styleFor(n.type).icon" :class="['h-8 w-8', styleFor(n.type).iconClass]"
                aria-hidden="true" />
                <div class="ml-3 w-0 flex-1">
                  <p :class="[styleFor(n.type).text.primary, 'text-sm font-medium']">
                    {{ t(n.statusTitle) }}
                  </p>
                  <p v-if="n.statusMessage !== undefined" :class="[styleFor(n.type).text.second, 'text-sm']">
                    {{ t(n.statusMessage) }}
                  </p>
                </div>
                  <button type="button" @click="notificationStore.hideNotification(n.id)"
                    class="ml-4 shrink-0 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                  <span class="sr-only">Close</span>
                  <XMarkIcon :class="['h-5 w-5', 'flex-none', styleFor(n.type).button]" aria-hidden="true"/>
                </button>
              </div>
            </div>
          </div>
